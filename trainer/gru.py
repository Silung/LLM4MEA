...
class GRUTrainer(metaclass=ABCMeta):
    ...
    def train_one_epoch(self, epoch, accum_iter):
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            ...
            # output sampling
            # logit_sampled = logit[:, labels.view(-1)]
            # loss = self.loss_func(logit_sampled, Variable(torch.arange(batch_size).long()))
            loss = self.loss_func(logits, labels.squeeze())
            loss.backward()
            self.optimizer.step()
    ...
    def validate(self, epoch, accum_iter):
        ...
        with torch.no_grad():
            ...
            hidden = self.model.init_hidden()
            for batch_idx, batch in enumerate(tqdm_dataloader):
                metrics, hidden = self.calculate_metrics(batch, hidden)
    ...
    def calculate_metrics(self, batch, hidden):
